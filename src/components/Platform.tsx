...
import Tailored_Simulations from "../../public/images/Tailored_Simulations.jpg";
import character from "../../public/images/ai_characters.jpg";
import evaluation from "../../public/images/ai_evaluation.jpg";
...
export default function Platform() {
  return (
    <section className="bg-[#001b27] py-10 text-white">
          ...
          {PlatformData.map((platform, index) => (
            <Fragment key={platform.heading}>
              <div className="space-y-2">
                <h3 className="text-xl font-semibold">{platform.heading}</h3>
                <p className="text-lg">{platform.paragraph}</p>
              </div>
              <Image
                src={platform.image}
                alt={platform.heading}
                className={cn("rounded-md", { "row-start-2": index % 2 !== 0 })}
              />
            </Fragment>
          ))}
        </div>
